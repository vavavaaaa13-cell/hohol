...
local function IsPlayerVisible(Player)
    ...
    local targetPartSetting = Options and Options.TargetPart and Options.TargetPart.Value
    local PlayerRoot = FindFirstChild(PlayerCharacter, targetPartSetting or "HumanoidRootPart") or FindFirstChild(PlayerCharacter, "HumanoidRootPart")
...
local function getClosestPlayer()
    local targetPartSetting = Options and Options.TargetPart and Options.TargetPart.Value
    if not targetPartSetting then return end
    local Closest
    local DistanceToMouse
    local ignoredPlayers = Options and Options.PlayerDropdown and Options.PlayerDropdown.Value or nil 

    for _, Player in next, GetPlayers(Players) do
        if Player == LocalPlayer then continue end
        if ignoredPlayers and ignoredPlayers[Player.Name] then continue end
        if (Toggles and Toggles.TeamCheck and Toggles.TeamCheck.Value) and Player.Team == LocalPlayer.Team then continue end
        ...
        local radiusLimit = (Options and Options.Radius and Options.Radius.Value) or 2000
        if Distance <= (DistanceToMouse or radiusLimit) then
            Closest = ((targetPartSetting == "Random" and Character[ValidTargetParts[math.random(1, #ValidTargetParts)]]) or Character[targetPartSetting])
...
local function loadLocalOrRemote(localFile, remoteUrl)
    local ok, src
    if typeof(isfile) == "function" and isfile(localFile) and typeof(readfile) == "function" then
        ok, src = pcall(readfile, localFile)
...
oldNamecall = hookmetamethod(game, "__namecall", newcclosure(function(...)
    ...
    if SilentAimSettings.CheckForFireFunc and (Method == "FindPartOnRay" or Method == "FindPartOnRayWithWhitelist" or Method == "FindPartOnRayWithIgnoreList" or Method == "Raycast" or Method == "ViewportPointToRay" or Method == "ScreenPointToRay") then
        ...
        if (Toggles and Toggles.aim_Enabled and Toggles.aim_Enabled.Value) and self == workspace and not checkcaller() and chance then
        ...
        if HitPart then
            ...
            if Method == "FindPartOnRayWithIgnoreList" and Options and Options.Method and Options.Method.Value == Method then
            ...
            elseif Method == "FindPartOnRayWithWhitelist" and Options and Options.Method and Options.Method.Value == Method then
                if ValidateArguments(Arguments, ExpectedArguments.FindPartOnRayWithWhitelist) then
                    local Origin, Direction = modifyRay(Arguments[2].Origin)
                    Arguments[2] = Ray.new(Origin, Direction * SilentAimSettings.MultiplyUnitBy)
                    return oldNamecall(unpack(Arguments))
                end
            elseif (Method == "FindPartOnRay" or Method == "findPartOnRay") and Options and Options.Method and Options.Method.Value and Options.Method.Value:lower() == Method:lower() then
                if ValidateArguments(Arguments, ExpectedArguments.FindPartOnRay) then
                    local Origin, Direction = modifyRay(Arguments[2].Origin)
                    Arguments[2] = Ray.new(Origin, Direction * SilentAimSettings.MultiplyUnitBy)
                    return oldNamecall(unpack(Arguments))
                end
            elseif Method == "Raycast" and Options and Options.Method and Options.Method.Value == Method then
            ...
            elseif Method == "ViewportPointToRay" and Options and Options.Method and Options.Method.Value == Method then
            ...
            elseif Method == "ScreenPointToRay" and Options and Options.Method and Options.Method.Value == Method then
...
local function enableBulletHitManipulation(value)
    ...
    if BManipulation then
        if not bulletHookEnabled then
            ...
            bulletOldNamecall = hookmetamethod(remote, "__namecall", newcclosure(function(self, ...)
                if typeof(self) == "Instance" then
                    ...
                    if method and (method == "FireServer" and self == remote) then
                        ...
                        if HitPart then
                            ...
                            return bulletOldNamecall(self, unpack(remArgs))
                        else
                            setnamecallmethod(method)
                        end
                    end
                end
                return bulletOldNamecall(self, ...)
            end))
        end
    else
        BManipulation = false
        if bulletHookEnabled then
            ...
            if bulletOldNamecall then
                hookmetamethod(remote, "__namecall", bulletOldNamecall)
            end
        end
    end
end
...
local function enableRocketHitManipulation(value)
    ...
    if RManipulation and not rocketHookEnabled then
        rocketHookEnabled = true
        rocketOldNamecall = hookmetamethod(remote, "__namecall", newcclosure(function(self, ...)
            if typeof(self) == "Instance" and getnamecallmethod() == "FireServer" and self == remote then
                ...
                if targetPart then
                    ...
                    return rocketOldNamecall(self, unpack(remArgs))
                end
            end
            return rocketOldNamecall(self, ...)
        end))
    elseif not RManipulation and rocketHookEnabled then
        rocketHookEnabled = false
        if rocketOldNamecall then hookmetamethod(remote, "__namecall", rocketOldNamecall) end
    end
end
...
local function GetActiveWeapon()
    if not masterToggle then return end
    return (P.Character or P.CharacterAdded:Wait()):FindFirstChild("Stinger")
end

local function FireRocket()
    if not ExplosiveEnabled then return end
    local weapon = GetActiveWeapon()
    if weapon then
        local targetPosition = M.Hit.Position
        local RS = game:GetService("ReplicatedStorage").RocketSystem.Events
        RS.FireRocket:InvokeServer(C.CFrame.LookVector, weapon, weapon, C.CFrame.Position, nil, workspace.Terrain)
        RS.RocketHit:FireServer(targetPosition, C.CFrame.LookVector, weapon, weapon, workspace.Terrain, workspace.Terrain, P.Name .. "Rocket")
    end
end
...
WarTycoonBox:AddToggle("ExplosiveEnabled", {
    Text = "Stinger click",
    Default = false,
    Tooltip = "shit function",
    Callback = function(state)
        ExplosiveEnabled = state
    end
})

WarTycoonBox:AddDropdown("ExplosiveMode", {
    AllowNull = false,
    Multi = false,
    Text = "Select Mode",
    Default = "Click",
    Values = {"Click", "Click Hold Spam"},
    Tooltip = "Choose click mode.",
    Callback = function(value)
        ExplosiveMode = value
    end
})
